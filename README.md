...
Because road conditions span widely varying frequencies, this project dynamically computes sample weights per class during dataloading. In `configs/default.yaml`, control these settings:

- `loss_name`: Choose `"weighted_cross_entropy"` (default) or `"class_balanced_focal"`.
- `class_weight_mode`: Choose between `"inverse"`, `"median_frequency"`, or `"normalized_inverse"`.
...
```
The `--export-gradcam` flag automatically wraps the model in a custom `pytorch-grad-cam` handler targeting the final fusion convolution. Results including accuracy, macro F1, and confusion matrices are logged in the terminal and saved to CSV.
...
4. **ROC & Recall-Confidence Curves**: Multiclass ROC with macro/micro AUC and practical Precision/Recall confidence sweep data.
...
```
*Note: Because this project tackles classification rather than semantic segmentation, Decoder heatmaps represent the multi-scale classification fusion path rather than expanding dense map outputs.* All figures are saved under `outputs/visualizations/{split}/` and logically split by functional directory.
